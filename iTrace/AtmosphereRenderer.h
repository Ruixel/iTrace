...
#define jSteps 8
			...
			SkyResult atmosphere(vec3 r, vec3 r0, vec3 pSun, float iSun, float rPlanet, float rAtmos, vec3 kRlh, float kMie, float shRlh, float shMie, float g) {
				...
				Result.WithMie = iSun * (pRlh * kRlh * totalRlh + 3.0f * mix(vec3(1, 0.721568627, 0.0745098039),vec3(1),0.55f) * pow(pMie / 3.0f, 1.0f) * 3.0f * kMie * totalMie);
			...
			Vector3f GetSunColor(Vector3f Direction) {
