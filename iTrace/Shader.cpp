...
iTrace::Rendering::Shader::Shader(const std::string& vertex, const std::string& geometry, const std::string& fragment) :
	VertexShader(LoadShader(GL_VERTEX_SHADER, vertex, VertexBuffer, VertexShader)),
...
void iTrace::Rendering::Shader::Bind() {
	glUseProgram(ShaderID);
}

void iTrace::Rendering::Shader::UnBind() {
	glUseProgram(NULL);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, int Value) const {
	glUniform1i(glGetUniformLocation(ShaderID, Name.c_str()), Value);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, size_t Value) const {
	glUniform1i(glGetUniformLocation(ShaderID, Name.c_str()), Value);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, float Value) const {
	glUniform1f(glGetUniformLocation(ShaderID, Name.c_str()), Value);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, bool Value) const {
	glUniform1i(glGetUniformLocation(ShaderID, Name.c_str()), Value);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector2f Value) const {
	glUniform2f(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector3f Value) const {
	glUniform3f(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y, Value.z);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector4f Value) const {
	glUniform4f(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y, Value.z, Value.w);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector2i Value) const {
	glUniform2i(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector3i Value) const {
	glUniform3i(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y, Value.z);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Vector4i Value) const {
	glUniform4i(glGetUniformLocation(ShaderID, Name.c_str()), Value.x, Value.y, Value.z, Value.w);
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Matrix4f Value, bool Transpose) const {
	glUniformMatrix4fv(glGetUniformLocation(ShaderID, Name.c_str()), 1, Transpose, glm::value_ptr(Value));
}

void iTrace::Rendering::Shader::SetUniform(const std::string& Name, Matrix3f Value, bool Transpose) const {
	glUniformMatrix3fv(glGetUniformLocation(ShaderID, Name.c_str()), 1, Transpose, glm::value_ptr(Value));
}

void iTrace::Rendering::Shader::SetUniformImageArray(const std::string& Name, int BaseImage, int Length)
{
	for (int i = 0; i < Length; i++) {

		std::string IndexName = Name + "[" + std::to_string(i) + "]";

		SetUniform(IndexName, BaseImage + i); 
...
void iTrace::Rendering::Shader::Reload(const std::string& vertex, const std::string& fragment, const std::string& geometry) {
	glDetachShader(ShaderID, VertexShader);
	glDetachShader(ShaderID, FragmentShader);
