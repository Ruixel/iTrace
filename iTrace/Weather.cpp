...
namespace iTrace {
	void WeatherManager::PrepareWeather()
	{
		...
		Weathers[static_cast<int>(Weather::OVERCAST)] = WeatherData{
			1.0, 1.0, //<- still the same type of clouds 
			3.0, //<- the global power is a lot smaller, resulting in generally more clouds
			2.0, //<- the detail power is also a lot smaller, resulting in generally larger clouds 
		...
		Weathers[static_cast<int>(Weather::RAINY)] = WeatherData{
			0.5, 10.0, //<- the clouds are a LOT darker 
			// ... The cloud shape is the same as overcast weather ... 
			3.0,
			2.0,
			4.0,

			0.5, 15.0, //<- the volumetrics are also a lot darker 
	...
	void WeatherManager::PollWeather(float t)
	{
		...
		CurrentWeatherFactor = glm::simplex(Vector2f(t, 0.0)) * 2 + 2; 
