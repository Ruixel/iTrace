...
namespace iTrace {
	namespace Core {
		...
		AABB3DAxis CollisionAABB::GetMajorAxis(Vector3f Position)
		{
			AABB3DAxis Axis = AABB3DAxis::POSX;
			...
			if (abs(Vector.x) > (abs(Vector.y)+.05) && abs(Vector.x) > abs(Vector.z)) {
				...
				Axis = Vector.z > 0.0 ? AABB3DAxis::POSZ : AABB3DAxis::NEGZ;
			}
		...
		bool CollisionAABB::IsOverlapping(Vector3f Position)
		{
		...
		bool CollisionAABB::HandleCollision(Vector3f& Position, AABB3DAxis* side)
		{
			if (IsOverlapping(Position)) {
				...
				case AABB3DAxis::POSX:
					Position.x = Max.x;
					break;
				case AABB3DAxis::NEGX:
					Position.x = Min.x;
					break;
				case AABB3DAxis::POSY:
					Position.y = Max.y;
					break;
				case AABB3DAxis::NEGY:
					Position.y = Min.y;
					break;
				case AABB3DAxis::POSZ:
					Position.z = Max.z;
					break;
				case AABB3DAxis::NEGZ:
					Position.z = Min.z;
					break;
		...
		void CollisionAABB::SetCenter(Vector3f Position)
		{
			Vector3f Size = Max - Min;
		...
		void CollisionAABB::SetSize(Vector3f Size)
		{
		...
		CollisionAABB::CollisionAABB(Vector3f Center, Vector3f Size)
		{
			Max = Center + Size / 2.f; 
			Min = Center - Size / 2.f; 
		}
