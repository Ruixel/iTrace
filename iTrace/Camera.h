...
namespace iTrace {
		...
		struct Camera {
			...
			inline void SetPosition(Vector3f Position) {
				...
				PrevView = View;
				View = iTrace::Core::ViewMatrix(Position, Rotation);
			}
			...
			inline void Move(Vector3f PositionAddon) {
				PrevView = View;
				View = iTrace::Core::ViewMatrix(Position + PositionAddon, Rotation);
			}
			inline void Rotate(Vector3f RotationAddon) {
				PrevView = View;
				View = iTrace::Core::ViewMatrix(Position, Rotation + RotationAddon);
			}
